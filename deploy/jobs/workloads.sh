...
# =================================================================
echo "Saving artifacts to Git LFS..."

# Add all new and modified files to Git staging.
# This includes the results, MLflow runs, and any other generated artifacts.
git add .

# Create a commit. The `-m` flag provides the message.
# We are creating a record of this specific job run.
git commit -m "chore(jobs): save artifacts from workload run on $(date)"

# Push the commit and the LFS objects to the remote repository.
git push
